...
class BaseResnetDistiller(BaseModel):
    ...
    def __init__(self, opt):
        assert opt.isTrain
        valid_netGs = ['resnet_9blocks', 'mobile_resnet_9blocks', 'super_mobile_resnet_9blocks', 'sub_mobile_resnet_9blocks'
                      'resnet_6blocks', 'mobile_resnet_6blocks', 'super_mobile_resnet_6blocks', 'sub_mobile_resnet_6blocks']
        ...
        elif opt.recon_loss_type == 'lpips':
            self.criterionRecon = models.modules.loss.LPIPSLoss().to(self.device)
        else:
            raise NotImplementedError('Unknown reconstruction loss type [%s]!' % opt.loss_type)
        
        block = int(opt.student_netG[-7])*3
        if isinstance(self.netG_teacher, nn.DataParallel):
            self.mapping_layers = ['module.model.%d' % i for i in range(9, block, 3)]
        else:
            self.mapping_layers = ['model.%d' % i for i in range(9, block, 3)]
        ...
        self.inception_model = create_metric_models(opt, device=self.device)
